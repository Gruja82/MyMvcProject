...
{
    public class ValidateMaterialName:ValidationAttribute
    {
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            // Fetch all Material names from database
            List<string> nameList = context.Materials.Select(e => e.Name).ToList();
            ...
            if (value != null && value.ToString() != String.Empty)
            {
                ...
                if (model.Id > 0)
                {
                    ...
                    if (model.Name != materialItem.Name)
                    {
                        // If supplied name is already contained in any of Material records then return error message
                        if (nameList.Contains(value.ToString()))
                        {
                            return new ValidationResult("There is already Material with this name in database. Please provide different one!");
                        }
            ...
            {
                return new ValidationResult("Please provide Material name!");
            }
        }
    }
}
